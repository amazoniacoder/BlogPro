...
/**
 * Transfer Russian dictionary words from source to target directory
 */
class RussianDictionaryTransfer {
  ...
  async transfer() {
    ...
    const words = await this.processSourceFile();
    await this.writeWordsFile(words);
    const metadata = await this.writeMetadataFile(words);
